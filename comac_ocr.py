...
def _OCR(img):
    _, result, angle = model.model(img,
                                        ...
                                        config=dict(MAX_HORIZONTAL_GAP=36,  # 字符之间的最大间隔，用于文本行的合并
                                        MIN_V_OVERLAPS=0.6,
                                        MIN_SIZE_SIM=0.6,
...
points_from = []
points_to = []
result_dict = {line['text']:[line['cx'],line['cy'],line['w'],line['h']] for line in result}
for k, v in boundboxes.items():
    if k[:3] != 'key':
        continue
    a = process.extractOne(k[3:], result_dict.keys(), scorer=fuzz.ratio, score_cutoff=90)
    if a:
        points_from.append([result_dict[a[0]][0],result_dict[a[0]][1]])
        points_to.append([v[0],v[1]])
        # print(points_from)
        # print(points_to)
affine_transform_matrix = cv2.estimateAffine2D(np.array(points_from),np.array(points_to))
# print(affine_transform_matrix[0])
dstImg = cv2.warpAffine(img,affine_transform_matrix[0],(w,h), borderMode=cv2.INTER_LINEAR, borderValue=cv2.BORDER_REPLICATE)
...
for key, value in boundboxes.items():
    ...
    for k,v in result_dict.items():
        if v[0]-v[2]/2>value[0]-value[2]/2 and v[0]+v[2]/2<value[0]+value[2]/2 and v[1]-v[3]/2 > value[1]-value[3]/2 and v[1]+v[3]/2 < value[1]+value[3]/2:
            if '口' in k or k is '冈':
                continue
            structure_data[key] += k
csv_data = csv_data.append(structure_data, ignore_index=True)
csv_data.to_csv('test/1.csv',sep=';',encoding='utf-8')